...
extension Kindred {
  ...
  static var example: Kindred {
    guard let url = Bundle.main.url(forResource: "Nadea Theron", withExtension: "pdf"),
          let pdf = CharacterPDF(url: url) else {
      fatalError("Unable to generate preview.")
    }
    return CharacterImporter.importCharacter(pdf: pdf, dataController: DataController.preview)
  }
